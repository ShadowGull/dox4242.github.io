(function(window, document, $, undefined) {
  ...
  var trophyIDs = {
    autocastSeries: [8, 9, 10, 11, 12, 13, 159, 200, 326],
  ...
  var upgradeIDs = {
    ...
    spellcraft: [130300, 129901, 130903, 130002, 130504, 129805, 130806, 130208, 130407, 130110, 130709, 130611, 129712, 144713, 145314, 144915, 145116, 145217, 145018, 144819, 153920, 153621, 153722, 153823, 154024, 162925, 163026, 162827, 171828, 171729, 205530, 205231, 205332, 205433, 209234, 209335, 209136, 209037, 216438, 216539, 218240, 218341],
    craftsmanship: [125300, 125201, 125702, 125903, 125004, 126305, 126106, 126007, 126208, 125409, 125610, 125111, 125812, 142613, 143214, 142815, 143116, 142717, 143018, 142919, 151520, 151421, 151622, 151323, 151224, 162125, 162026, 161927, 171128, 171229, 203930, 203831, 203632, 203733, 207634, 207535, 207436, 207737, 215738, 215639, 217540, 217441],
    divine: [126400, 127001, 126602, 126803, 127504, 126905, 127206, 127108, 127409, 126507, 127610, 127311, 126712, 143813, 143614, 143915, 143316, 143517, 143418, 143719, 152720, 152621, 152922, 153023, 152824, 162225, 162426, 162327, 171328, 171429, 204230, 204331, 204132, 204033, 208034, 208135, 207936, 207837, 215938, 215839, 217740, 217641],
    economics: [128100, 128701, 128202, 128403, 127804, 128305, 127907, 128806, 128608, 127709, 128510, 128011, 128912, 144413, 144214, 144615, 144516, 144017, 144318, 144119, 153320, 153121, 153522, 153223, 153424, 162525, 162626, 162727, 171528, 171629, 204430, 204731, 204532, 204633, 208434, 208235, 208336, 208537, 216138, 216039, 217840, 217941],
    alchemy: [124500, 124801, 123903, 124002, 124304, 124605, 124206, 124907, 123808, 124109, 124710, 123711, 124412, 142313, 142414, 142115, 142216, 141917, 142518, 142019, 152420, 152521, 152122, 152323, 152224, 161725, 161626, 161827, 171028, 170929, 203430, 203231, 203532, 203333, 207334, 207035, 207236, 207137, 215538, 215439, 217240, 217341],
    warfare: [131000, 131501, 132202, 131603, 131204, 132005, 131810, 131909, 131706, 131407, 131108, 131311, 132112, 145413, 145514, 145615, 145816, 145717, 146018, 145919, 154120, 154421, 154222, 154323, 154524, 163125, 163326, 163227, 171928, 172029, 205930, 205731, 205632, 205833, 209634, 209735, 209536, 209437, 216738, 216639, 218440, 218541],
	forbidden: [204900, 205001, 204802, 205103, 208604, 208905, 208806, 208707, 216308, 216209, 218010, 218111],
    ...
    dbTiers: [402101, 402102, 402103, 402104],
    ssTiers: [401701, 401702, 401703, 401704],
    tfTiers: [402501, 402502, 402503, 402504],
    acTiers: [402601, 402602, 402603, 402604],
    msTiers: [402701, 402702, 402703, 402704],
    pcTiers: [403001, 403002, 403003, 403004],
    isTiers: [402801, 402802, 402803, 402804],
    lwTiers: [402901, 402902, 402903, 402904],
  };
  ...
  $(function() {
    ...
    Vue.component('widget-fivestat-header', {
      ...
      + '<th><span class="statheader">This Era</span></th>'
      + '<th><span class="statheader">Prior Eras</span></th>'
    ...
    Vue.component('widget-unique-header', {
      template: '<tr>'
      + '<th><span class="statheader">Building</span></th>'
      + '<th><span class="statheader">This Reincarnation</span></th>'
      + '<th><span class="statheader">Prior Reincarnations</span></th>'
      + '<th><span class="statheader">All Reincarnations</span></th>'
      + '</tr>'
    ...
    Vue.component('widget-unique-max', {
      props: ['building', 'name', 'id'],
      data: function () {
        return {
            refm: "um" + this.id,
            refe: "ue" + this.id,
      ...
      template: '<tr>'
      + '<th><span class="statname">{{name}}</span></th>'
      + '<td><input v-model="building[refm]" number></input></td>'
      + '<td><input v-model="building[refe]" number></input></td>'
      + '<td><span class="derivedstat">{{total}}</span></td>'
      + '</tr>',
      computed: {
        total: function() {
          return Math.max(this.building[this.refm], this.building[this.refe]);
        }
      }
    });
    ...
    Vue.component('widget-bloodline-dropdown', {
      ...
      computed: {
        unlocked: {
          get: function() {
            var blFactionIDs = { 0:194, 1:164, 2:39, 3:212, 4:396, 5:103, 6:380, 7:136, 8:183, 9:150, 10:120, 12:598, 13:939, 14:940, 15:953 };
    ...
    Vue.component('widget-faction-upgrade-header', {
      ...
      + '<th><span></span></th>'
      + '<th><span>Advanced Heritage</span></th>'
      + '</tr>'
    });
	...
	Vue.component('widget-research-upgrades-header2', {
      template: '<tr>'
    ...
    Vue.component('widget-research-upgrade-row2', {
      props: {
        'upgrades': Object,
        'f_name': String,
        'f_id': String,
      ...
      template: '<tr>'
	  + '<td><span class="statname">{{f_name}}</span> '
    ...
    Vue.component('widget-legacy-header', {
      template: '<tr>'
      + '<th><span class="statheader">Faction</span></th>'
      + '<th><span class="statheader">Bronze</span></th>'
      + '<th><span class="statheader">Silver</span></th>'
      + '<th><span class="statheader">Gold</span></th>'
      + '<th><span class="statheader">Diamond</span></th>'
      + '</tr>'
    });
    
    Vue.component('widget-legacy', {    //Legacies have sequential IDs (Except Undead Diamond)
      props: {
        'upgrades': Object,
        'name': String,
        'id': String
      ...
      template: '<tr>'
      + '<th><span class="statname">{{name}}</span></th>'
      + '<td><input type="checkbox" v-model="bronze" number></input></td>'
      + '<td><input type="checkbox" v-model="silver" number></input></td>'
      + '<td><input type="checkbox" v-model="gold" number></input></td>'
      + '<td><input type="checkbox" v-model="diamond" number></input></td>'
      + '</tr>',
      methods: {
        get: function(id) {
          if (this.upgrades[id]) {return true; }
      ...
      computed: {
    	bronze: {
          ...
          set: function(x) {
            return this.set(this.id,x);
          }
        },
        silver: {
          get: function() {
            return this.get(1 + parseInt(this.id) + "");
      	  },
          set: function(x) {
            return this.set(1 + parseInt(this.id) + "",x);
          }
        },
        gold: {
          get: function() {
            return this.get(2 + parseInt(this.id) + "");
      	  },
          set: function(x) {
            return this.set(2 + parseInt(this.id) + "",x);
          }
        },
        
        diamond: {    //Undead legacy 4's id is the only non-sequential ID.
          get: function() {
            if (this.name === "Undead") {return this.get("1126")};
            return this.get(3 + parseInt(this.id) + "");
      	  },
          set: function(x) {
            if (this.name === "Undead") {return this.set("1126",x)};
            return this.set(3 + parseInt(this.id) + "",x);
          }
        },
      }
    });
